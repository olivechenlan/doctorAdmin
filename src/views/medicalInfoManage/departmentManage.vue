  ...
  <div class="app-container">
    ...
    >
      ...
      <el-table-column label="科室状态" min-width="150" align="center">
        <template slot-scope="{row}">
          <span>{{ row.useState|formatToState }}</span>
        </template>
      </el-table-column>
          ...
          <el-col :span="18">
            ...
            <el-form-item v-if="temp.type==='1'" label="父级科室" prop="parDepartment">
              <el-select v-model="temp.parDepartment" placeholder="请选择一级科室" clearable>
                <el-option v-for="item in departmentOptions" :disabled="item.departmentId===temp.departmentId" :key="item.departmentId" :label="item.departmentName" :value="item.departmentId" />
              </el-select>
            </el-form-item>
...
export default {
  filters: {
    formatToType(state) {
      let result = { name: '' }
      result = !!state && (map.getDepartmentType.find(item => item.code === state))
      return result.name
    },
    formatToState(state) {
      let result = { name: '' }
      result = !!state && (map.getDepartmentStatus.find(item => item.code === state))
      return result.name
    }
  },
  components: { headline },
  data() {
    return {
      ...
      departmentStateOptions: map.getDepartmentStatus,
      departmentOptions: [],
      textMap: {
        update: '编辑科室信息',
        create: '新增科室'
